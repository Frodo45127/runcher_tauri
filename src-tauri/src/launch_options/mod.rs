...
impl LaunchOptions {
    ...
    pub fn prepare_launch_options(&mut self, app: &AppHandle, options: &[LaunchOption], game: &GameInfo, data_path: &Path, folder_list: &mut String) -> Result<()> {
        ...
        if self.options.iter().any(|option| option.enabled) {
            ...
            // Logging check.
            if self.options.iter().find(|option| option.key == "enable_logging").unwrap().enabled {
                cmd.arg("-e");
            }

            // Skip Intros check.
            if self.options.iter().find(|option| option.key == "skip_intros").unwrap().enabled {
                cmd.arg("-i");
            }

            // Remove Trait Limit check.
            if self.options.iter().find(|option| option.key == "remove_trait_limit").unwrap().enabled {
                cmd.arg("-r");
            }

            // Remove Siege Attacker check.
            if self.options.iter().find(|option| option.key == "remove_siege_attacker").unwrap().enabled {
                cmd.arg("-a");
            }

            // Enable Dev-only UI check.
            if self.options.iter().find(|option| option.key == "enable_dev_only_ui").unwrap().enabled {
                cmd.arg("-d");
            }

            // Translations check.
            if let Some(option) = self.options.iter().find(|option| option.key == "enable_translations") {
                if option.enabled {
                    if let Some(param) = option.parameters.iter().find(|param| param.key == "language") {
            ...
            if let Some(option) = self.options.iter().find(|option| option.key == "universal_rebalancer") {
                if option.enabled {
                    if let Some(param) = option.parameters.iter().find(|param| param.key == "base_mod") {
            ...
            if let Some(option) = self.options.iter().find(|option| option.key == "unit_multiplier") {
                if option.enabled {
                    if let Some(param) = option.parameters.iter().find(|param| param.key == "multiplier") {
            ...
            self.options.iter()
                ...
                .filter_map(|option| if let Some(script) = self.scripts.get(&option.key) {
                    Some((script, option))
                } else {
                    None
                    ...
                    let script_params = if script.metadata().parameters().is_empty() {
                        ...
                        let preset = if let Some(preset_value) = preset_value {
                            if preset_value != "none" && sql_folder_presets.is_dir() {
                                files_from_subdir(&sql_folder_presets, false).unwrap()
                                    .iter()
                        ...
                        match preset {
                            ...
                            None => {
                                for param in script.metadata().parameters() {
                                    let key = format!("{}:{}:{}", game.key(), option.key, param.key());
            ...
            #[cfg(target_os = "windows")] cmd.creation_flags(DETACHED_PROCESS);
    ...
    pub fn generate_options(&mut self, app: &AppHandle, game: &GameInfo, game_path: &Path) -> Result<Vec<LaunchOption>> {
        self.options.clear();
        self.scripts.clear();
        ...
        if path_is_valid {
            ...
            if let Some(ref mut translations_option) = self.options.iter_mut().find(|option| option.key == "enable_translations") {
                if let Some(ref mut language_param) = translations_option.parameters.iter_mut().find(|param| param.key == "language") {
                    if let Ok(ca_packs) = game.ca_packs_paths(game_path) {
                        ...
                        if !languages.is_empty() {
                            language_param.value = LaunchOptionValue::Select(languages[0].clone(), languages.clone());
            ...
            if let Some(ref mut universal_rebalancer_option) = self.options.iter_mut().find(|option| option.key == "universal_rebalancer") {
                    ...
                    if let Some(mut game_config) = game_config {
                        if let Ok(game_data_path) = game.data_path(game_path) {
                            ...
                            if !packs_for_rebalancer.is_empty() {
                                base_mod_param.value = LaunchOptionValue::Select(packs_for_rebalancer[0].clone(), packs_for_rebalancer.clone());
                                base_mod_param.default = LaunchOptionValue::Text(packs_for_rebalancer[0].clone());
                            }
                        }
                    }
                }
                ...
                if let Some(extension) = path.extension() {
                    ...
                    if extension == "yml" {
                        if let Ok(script) = SQLScript::from_path(&path) {
                            let presets = presets_by_script.get(script.metadata().key())
                            ...
                            self.scripts.insert(script.metadata().key().to_string(), script);
                        }
                    }
                }
            }
                    ...
                    for param in &mut option.parameters {
                        ...
                        if let Some(value) = settings.launch_options.get(&key) {
                            param.value = match param.value {
                                LaunchOptionValue::Boolean(_) => LaunchOptionValue::Boolean(value.parse::<bool>().unwrap_or_default()),
                                LaunchOptionValue::Number(_) => LaunchOptionValue::Number(value.parse::<f64>().unwrap_or_default()),
                                LaunchOptionValue::Text(_) => LaunchOptionValue::Text(value.clone()),
                                LaunchOptionValue::Select(_, _) => LaunchOptionValue::Select(value.clone(), vec![]),
                            };
                        } else {
                            param.value = match &param.default {
                                LaunchOptionValue::Boolean(default) => LaunchOptionValue::Boolean(*default),
                                LaunchOptionValue::Number(default) => LaunchOptionValue::Number(*default),
                                LaunchOptionValue::Text(default) => match &param.value {
                                    LaunchOptionValue::Select(_, available_values) => if available_values.contains(default) {
                                        LaunchOptionValue::Select(default.to_owned(), available_values.clone())
                                    } else {
                                        param.value.clone()
                                    },
                                ...
                                LaunchOptionValue::Select(default, _) => LaunchOptionValue::Select(default.to_owned(), vec![]),
                            };
                        }
                    }
                }
